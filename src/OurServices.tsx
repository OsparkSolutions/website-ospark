...
export const OurServices = () => {
    ...
    return (
        <div>
            ...
            <MyComponent frontTitle='Mobile Web & App Development' mainImg={phone3}  isImageLeft={true} boxes={boxes1}/>
            <MyComponent frontTitle='Website Design & Management' mainImg={startup} isImageLeft={false} boxes={boxes2} />
            <MyComponent frontTitle='Reporting, BI & Data Warehousing' mainImg={reporting} isImageLeft={true} boxes={boxes3} />
...
const boxes2: BoxComponent[] = [{name: 'HTML', img: html, size: '70%', color: '#E54C21'},{name: 'CSS', img: css, size: '50%', color: '#2965F1'},{name: 'Azure', img: azure, size: '75%', color: '#33B2EB'}]
const boxes3: BoxComponent[] = [{name: 'Power BI', img: powerbi, size: '75%', color:'#E9B62F'},{name: 'Sql Server', img: sql, size: '60%', color: '#3298DA'},{name: 'Databricks', img: databricks, size: '75%', color: '#FF5224'}]

const MyComponent = (props: any) => {
    const { ref: myRef, inView: myElementIsVisible } = useInView({ threshold: 1, triggerOnce: true });
    const flipCardRef = useRef<any>(null)
    const [backT, setBackT] = useState<string>('placeholder')
    return (
        <div className={mergeStyles({
            ...
            <div ref={flipCardRef} className={mergeStyles(styles.cardFlip.flipCard, { position: "relative" })}>
                <div className={mergeStyles(styles.cardFlip.flipCardInner, {
                    ...
                    <div className={mergeStyles(styles.cardFlip.flipCardBack, styles.boxedShadow, {
                        ...
                        <h3 className={mergeStyles(styles.fieldwork, { margin: "1rem" })}>{backT ? backT : 'Placeholder Back Title'}</h3>
                    </div>
                </div>
            </div>
            ...
            <div ref={myRef} className={mergeStyles(
                ...
                {props.boxes.map((box: BoxComponent, i: any) => {
                    ...
                    return <div style={{ aspectRatio: 1 }}
                        ...
                        {myElementIsVisible && <><div onMouseEnter={()=>{
                            setBackT(box.name)
                            flipCardRef.current.className = styles.flipper
                    }} onMouseLeave={()=>{
                        flipCardRef.current.className = mergeStyles(styles.cardFlip.flipCard, { position: "relative",transition: "transform 0.8s",
