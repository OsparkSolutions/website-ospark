...
export const OurServices = () =>{
    return(
        <div >
            <h1 className={styles.sectionHeader}><span className={styles.orange}>O</span>ur Services</h1>
            <OurServicesComponent title = 'Mobile & Web & Gaming App Development' uri={reporting} isImageRight={false}>
...
const OurServicesComponent = (props: PropsWithChildren<OurServicesComponent>) => {
   ...
   // let imageClass = windowSize.deviceSize <= DeviceSize.medium ? styles.largeScreenImageContainer : mergeStyles({float: props.isImageRight ? 'right' : 'left', width: '30%', display:'flex',alignItems: 'center', flexDirection:'column'});
    ...
    let textClass = windowSize.deviceSize <= DeviceSize.medium ? styles.mainSectionTitle : mergeStyles({fontFamily: 'fieldwork, sans-serif'});
